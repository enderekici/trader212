...
if [ -f .env ]; then
  ...
  echo -e "${YELLOW}WARNING: No .env file found locally. You'll need to create it on the server.${NC}"
fi
...
# Install Docker if not installed (includes compose plugin)
...
if ! docker compose version &> /dev/null; then
  echo "ERROR: docker compose plugin not found. Install Docker Engine >= 20.10"
  exit 1
fi
